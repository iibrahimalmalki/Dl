...
const ST={pending:{ar:"قيد المراجعة",color:"#b54708",bg:"#fef3e2"},shortlisted:{ar:"مرشح",color:"#175cd3",bg:"#eff6ff"},interview:{ar:"مقابلة",color:"#6941c6",bg:"#f5f3ff"},accepted:{ar:"مقبول",color:"#087443",bg:"#e7f7ef"},rejected:{ar:"مرفوض",color:"#b42318",bg:"#feecea"}};
const CL={strong:{ar:"مرشح قوي",ic:"star",color:"#087443"},accepted:{ar:"مقبول",ic:"check",color:"#175cd3"},needs_interview:{ar:"يحتاج مقابلة",ic:"search",color:"#b54708"},rejected:{ar:"غير مناسب",ic:"x",color:"#b42318"}};
  ...
  const upStatus=async(id,status)=>{
    ...
    if(status==="accepted"&&a){
      ...
      if(a.whatsapp){
        ...
        window.open(waLink(a.whatsapp,msg));
      }
    }
...
function Card({a,onClick,onAnalyze,analyzing,compareMode,selected}){
  const st=ST[a.status]||ST.pending;
  const cl=a.ai_classification?CL[a.ai_classification]:null;
  const geo=a.bangladesh_district?calcGeoScore(a.bangladesh_district):null;
  const riyadhWarn=a.location==="inside_ksa"&&a.ready_for_riyadh==="no";
  const loc=a.location==="inside_ksa"?(a.saudi_city||"داخل المملكة"):(a.bangladesh_district?`${a.bangladesh_district}${a.bangladesh_city?" — "+a.bangladesh_city:""}`:"خارج المملكة");
  const zoneDot=geo?{green:"#12b76a",yellow:"#f79009",red:"#f04438"}[geo.zone]:null;
  return(<div style={{...s.card,...(compareMode&&selected?{borderColor:"#6941c6",background:"#f5f3ff"}:{}),...(riyadhWarn?{borderInlineEnd:"3px solid #f04438"}:{})}} onClick={onClick}>
    {compareMode&&<div style={{marginBottom:6}}><input type="checkbox" checked={selected} readOnly style={{accentColor:"#6941c6"}}/></div>}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8,gap:10}}>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          <div style={{color:"#0f172a",fontSize:14.5,fontWeight:800}}>{a.full_name||"—"}</div>
          {a.application_number&&<div style={{background:"#fff2e8",color:"#E8712B",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:20,border:"1px solid #ffd9bd"}}>#{a.application_number}</div>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:5,color:"#64748b",fontSize:11.5,marginBottom:3}}><Icon n="phone" s={13}/><span dir="ltr">{a.whatsapp||"—"}</span>{a.age&&<span style={{color:"#cbd5e1"}}>·</span>}{a.age&&<span>{a.age} سنة</span>}</div>
        {(a.bangladesh_district||a.saudi_city||a.location)&&<div style={{display:"flex",alignItems:"center",gap:5,color:"#94a3b8",fontSize:11,marginBottom:4}}><Icon n="pin" s={13}/>{loc}</div>}
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {riyadhWarn&&<span style={{...s.chip,background:"#feecea",border:"1px solid #f7bfba",color:"#b42318"}}><Icon n="alert" s={12}/> غير مستعد للرياض</span>}
          {geo&&<span style={{...s.chip,background:ZONE_COLORS[geo.zone].bg,border:`1px solid ${ZONE_COLORS[geo.zone].border}`,color:ZONE_COLORS[geo.zone].text}}><i style={{width:7,height:7,borderRadius:"50%",background:zoneDot,display:"inline-block"}}/> {geo.score}/100</span>}
        </div>
      </div>
      <div>{a.ai_score_total!=null?<div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#E8712B,#f5a35f)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flex:"none"}}><div style={{color:"#fff",fontSize:15,fontWeight:900,lineHeight:1}}>{Number(a.ai_score_total).toFixed(1)}</div><div style={{color:"rgba(255,255,255,0.85)",fontSize:8.5}}>/10</div></div>:<div style={{width:48,height:48,borderRadius:14,background:"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center",color:"#cbd5e1",flex:"none"}}><Icon n="robot" s={20}/></div>}</div>
    </div>
    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
      <div style={{...s.chip,color:st.color,background:st.bg}}><span style={{width:6,height:6,borderRadius:"50%",background:st.color,display:"inline-block"}}/>{st.ar}</div>
      {cl&&<div style={{...s.chip,color:cl.color,background:"#f8fafc",border:"1px solid #eceef1"}}><Icon n={cl.ic} s={12}/>{cl.ar}</div>}
      <div style={{flex:1}}/>
      {!compareMode&&!a.ai_score_total&&<button onClick={e=>{e.stopPropagation();onAnalyze(a);}} disabled={analyzing} style={{padding:"6px 11px",background:"linear-gradient(135deg,#6941c6,#7c3aed)",border:"none",borderRadius:9,color:"#fff",fontSize:10.5,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5}}>{analyzing?"…":<><Icon n="robot" s={13}/> تحليل AI</>}</button>}
    </div>
  </div>);}
...
function StatsPanel({list,onBack}){
  ...
  return(<div style={s.root}>
    ...
    <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={s.sec}><div style={{color:"#1e293b",fontSize:13,fontWeight:800,marginBottom:10}}>حسب الشهر</div>{Object.entries(byMonth).map(([m,d])=><div key={m} style={{padding:"8px 0",borderBottom:"1px solid #f1f5f9"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:"#1e293b",fontSize:12,fontWeight:700}}>{m}</span><span style={{color:"#E8712B",fontWeight:800,fontSize:13}}>{d.total}</span></div><div style={{display:"flex",gap:12,fontSize:10.5}}><span style={{color:"#087443",fontWeight:700}}>مقبول: {d.accepted}</span><span style={{color:"#b42318",fontWeight:700}}>مرفوض: {d.rejected}</span></div></div>)}</div>
      <div style={s.sec}><div style={{color:"#1e293b",fontSize:13,fontWeight:800,marginBottom:10}}>أكثر المناطق تقديماً</div>{topGeo.map(([g,c])=><div key={g} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #f1f5f9"}}><span style={{color:"#475569",fontSize:12}}>{g}</span><span style={{color:"#2563eb",fontWeight:800,fontSize:13}}>{c}</span></div>)}</div>
    </div>
  </div>);
}
...
<div class="sec"><h3>تحليل الذكاء الاصطناعي</h3>
  ...
  ${ev?.weaknesses?.length?`<div class="note" style="border-color:#dc2626"><b>نقاط الضعف:</b><ul>${ev.weaknesses.map(x=>`<li>${x}</li>`).join("")}</ul></div>`:""}
  ...
  return(<div style={s.root}>
    ...
    <div style={{padding:"12px 16px 80px",display:"flex",flexDirection:"column",gap:12,maxWidth:560,margin:"0 auto"}}>
      ...
      <div style={s.sec}><div style={{color:"#1e293b",fontSize:13,fontWeight:800,marginBottom:12}}>الموقع والمنطقة</div>
        ...
        {a.bangladesh_city&&<a href={mapUrl} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:6,color:"#2563eb",fontSize:12,fontWeight:700,textDecoration:"none",marginTop:4}}><Icon n="pin" s={14}/> عرض على الخريطة</a>}
      </div>

      {/* البيانات */}
      <div style={s.sec}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{color:"#1e293b",fontSize:13,fontWeight:800}}>البيانات الشخصية</div><button onClick={()=>setEditMode(!editMode)} style={{padding:"5px 12px",background:editMode?"#f1f5f9":"linear-gradient(135deg,#E8712B,#d4631f)",border:"none",borderRadius:8,color:editMode?"#475569":"#fff",fontSize:11,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5}}>{editMode?<><Icon n="x" s={13}/> إلغاء</>:<><Icon n="edit" s={13}/> تعديل</>}</button></div>
      ...
      <div style={s.sec}><div style={{color:"#1e293b",fontSize:13,fontWeight:800,marginBottom:12}}>تحليل الذكاء الاصطناعي</div>
        ...
        {(aiReport||a.ai_evaluation_json)&&(()=>{const ev=aiReport||a.ai_evaluation_json;return(<div>
          ...
          {ev&&<>
            ...
            {ev.weaknesses?.length>0&&<EB c="#dc2626" t="نقاط الضعف" list={ev.weaknesses}/>}
...
return(<div style={{background:"#eff6ff",border:"1px solid #93c5fd",borderRadius:14,padding:14,marginTop:10}}><div style={{color:"#1d4ed8",fontSize:12,fontWeight:800,marginBottom:8}}>الأسئلة ({all.length}):</div>{weak.length>0&&<div style={{background:"#fef9c3",borderRadius:8,padding:"4px 10px",marginBottom:6,color:"#854d0e",fontSize:11,fontWeight:700}}>{weak.length} أسئلة متابعة</div>}{all.map((q,i)=><div key={i} style={{color:"#1e40af",fontSize:10,marginBottom:2}}>{i+1}. {q.ar}</div>)}<button onClick={gen} disabled={loading} style={{...s.bFull,background:"linear-gradient(135deg,#2563eb,#1d4ed8)",marginTop:10}}>{loading?"جاري...":"إنشاء الرابط"}</button></div>);}
function EmpCard({e,onClick}){const RQ=["date_of_birth","time_in_saudi","residence_riyadh","previous_job","car_wash_experience","worked_with_app"];const missCount=RQ.filter(k=>!e[k]||String(e[k]).trim()==="").length;return(<div style={s.card} onClick={onClick}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,gap:10}}><div style={{color:"#0f172a",fontSize:14.5,fontWeight:800}}>{e.full_name||"—"}</div><div style={{background:"#fff2e8",color:"#E8712B",fontSize:10,fontWeight:800,padding:"2px 9px",borderRadius:20,border:"1px solid #ffd9bd",flex:"none"}}>{e.employee_id||"—"}</div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 12px",marginBottom:missCount>0?8:0}}>{[[e.nationality,"globe"],[e.mobile,"phone"],[e.source,"inbox"],[e.time_in_saudi,"clock"]].map(([v,ic],i)=>v?<div key={i} style={{display:"flex",alignItems:"center",gap:6,color:"#64748b",fontSize:11.5,minWidth:0}}><span style={{color:"#94a3b8",display:"flex",flex:"none"}}><Icon n={ic} s={13}/></span><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} dir={ic==="phone"?"ltr":"rtl"}>{v}</span></div>:null)}</div>{missCount>0&&<div style={{...s.chip,background:"#fef3e2",border:"1px solid #fbdba7",color:"#b54708"}}><Icon n="alert" s={12}/> ملف ناقص — {missCount} حقول</div>}</div>);}
function EmpDetail({e,onBack,onDelete,onUpdate}){const[del,setDel]=useState(false);const[editMode,setEditMode]=useState(false);const[eSaving,setESaving]=useState(false);const[copied,setCopied]=useState(false);
  ...
  const sendCompletionRequest=()=>{
    ...
    window.open(waLink(e.mobile,msg));
  };
  const updateLink=`${SITE_URL}/?employee=${e.id}`;
  const copyLink=async()=>{try{await navigator.clipboard.writeText(updateLink);}catch{const ta=document.createElement("textarea");ta.value=updateLink;document.body.appendChild(ta);ta.select();document.execCommand("copy");ta.remove();}setCopied(true);setTimeout(()=>setCopied(false),2000);};
...
return(<div style={s.root}><div style={{background:"linear-gradient(135deg,#1e293b,#334155)",padding:"14px 20px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:100}}><button onClick={onBack} style={s.bSm}><Icon n="back" s={15} style={{verticalAlign:"-2px"}}/> رجوع</button><div style={{flex:1,textAlign:"right"}}><div style={{color:"#fff",fontSize:15,fontWeight:900}}>{e.full_name}</div><div style={{color:"rgba(255,255,255,0.6)",fontSize:11}}>ID: {e.employee_id||"—"}</div></div></div><div style={{padding:"12px 16px 80px",display:"flex",flexDirection:"column",gap:12,maxWidth:560,margin:"0 auto"}}>
  {missing.length>0&&<div style={{background:"#fffbeb",border:"1.5px solid #fcd34d",borderRadius:14,padding:14}}>
    ...
    <button onClick={sendCompletionRequest} style={{...s.bFull,background:"linear-gradient(135deg,#16a34a,#15803d)",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:7}}><Icon n="phone" s={15}/> إرسال طلب إكمال الملف عبر واتساب</button>
    <button onClick={copyLink} style={{...s.bFull,marginTop:8,background:"#fff",border:"1.5px solid #16a34a",color:"#16a34a",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:7}}><Icon n={copied?"check":"link"} s={15}/> {copied?"تم نسخ الرابط":"نسخ رابط التحديث"}</button>
    <div style={{marginTop:8,background:"#f8fafc",border:"1px solid #eceef1",borderRadius:8,padding:"7px 10px",fontSize:10.5,color:"#64748b",wordBreak:"break-all",direction:"ltr",textAlign:"left"}}>{updateLink}</div>
  </div>}
  <div style={s.sec}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <button onClick={()=>{setEd(Object.fromEntries(EDITABLE.map(k=>[k,e[k]||""])));setEditMode(!editMode);}} style={{padding:"5px 12px",background:editMode?"#f1f5f9":"linear-gradient(135deg,#E8712B,#f5a35f)",border:"none",borderRadius:8,color:editMode?"#475569":"#fff",fontSize:11,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5}}>{editMode?<><Icon n="x" s={13}/> إلغاء</>:<><Icon n="edit" s={13}/> تعديل</>}</button>
