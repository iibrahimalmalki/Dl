...
export default function UserManagement({onClose}){
  ...
  const openUser=async(u)=>{setSel(u);setMsg("");const{data}=await supabase.from("user_permissions").select("module,can_view,can_edit").eq("user_id",u.id);const m={};(data||[]).forEach(r=>{m[r.module]={view:r.can_view,edit:r.can_edit};});setPerms(m);};
  ...
  const savePerms=async()=>{if(!sel)return;setSaving(true);setMsg("");await supabase.from("user_permissions").delete().eq("user_id",sel.id);const rows=MODULES.filter(x=>perms[x.k]&&(perms[x.k].view||perms[x.k].edit)).map(x=>({user_id:sel.id,module:x.k,can_view:!!perms[x.k].view,can_edit:!!perms[x.k].edit}));if(rows.length){const{error}=await supabase.from("user_permissions").insert(rows);if(error){setMsg("خطأ: "+error.message);setSaving(false);return;}}setSaving(false);setMsg("✅ حُفظت الصلاحيات");};

  const createUser=async()=>{
    if(!add.email.trim()||add.password.length<6){setMsg("أدخل بريداً وكلمة مرور 6 أحرف على الأقل");return;}
    setSaving(true);setMsg("");const r=await call({action:"create",...add});setSaving(false);
    if(r.error){setMsg("❌ "+r.error);return;}
    setShowAdd(false);setAdd({email:"",display_name:"",password:"",biker_employee_id:""});setMsg("✅ أُنشئ المستخدم");loadUsers();
  };
  const doDelete=async(u)=>{if(!confirm(`حذف ${u.display_name||u.email} نهائياً؟`))return;setMsg("");const r=await call({action:"delete",user_id:u.id});if(r.error){setMsg("❌ "+r.error);return;}if(sel&&sel.id===u.id)setSel(null);setMsg("✅ حُذف");loadUsers();};
  const toggleActive=async(u)=>{const r=await call({action:"update",user_id:u.id,active:!u.active});if(r.error){setMsg("❌ "+r.error);return;}loadUsers();if(sel&&sel.id===u.id)setSel({...u,active:!u.active});};
  const savePw=async(u)=>{if(pwVal.length<6){setMsg("كلمة المرور 6 أحرف على الأقل");return;}setMsg("");const r=await call({action:"set_password",user_id:u.id,password:pwVal});if(r.error){setMsg("❌ "+r.error);return;}setPwFor(null);setPwVal("");setMsg("✅ تم تغيير كلمة المرور");};
  ...
  return(<div style={ov} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
    <div style={card}>
      <div style={hd}>
        <div><div style={{fontSize:16,fontWeight:800}}>👥 المستخدمون والصلاحيات</div><div style={{fontSize:11,color:"#8a97a6"}}>المالك يضيف/يحذف/يمنح ويغيّر كلمات المرور</div></div>
      ...
      <div style={{padding:16}}>
        {msg&&<div style={{fontSize:12,marginBottom:10,color:msg.startsWith("✅")?"#3fb950":"#f85149"}}>{msg}</div>}
        ...
        sel?(<>
          ...
          {!showAdd?<button onClick={()=>{setShowAdd(true);setMsg("");}} style={{width:"100%",padding:"11px",border:"1px dashed #3fb950",background:"rgba(63,185,80,.08)",color:"#3fb950",borderRadius:12,fontWeight:800,fontSize:13,cursor:"pointer",marginBottom:12}}>➕ إضافة مستخدم جديد</button>:
          ...
          {users.map(u=>(<div key={u.id} style={{background:"#141a22",border:"1px solid #232d39",borderRadius:12,padding:"10px 14px",marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                <button onClick={()=>{setPwFor(pwFor===u.id?null:u.id);setPwVal("");}} style={btn("#1c2530","#e8eef5")}>🔑 كلمة المرور</button>
                ...
                {!u.is_owner&&<button onClick={()=>openUser(u)} style={btn("linear-gradient(135deg,#E8712B,#d4631f)","#fff")}>الصلاحيات ←</button>}
          ...
          <div style={{marginTop:8,background:"#141a22",border:"1px dashed #232d39",borderRadius:12,padding:"10px 14px",fontSize:11.5,color:"#8a97a6"}}><b style={{color:"#e3b341"}}>🔒 بيانات TMA غير مدرجة في الصلاحيات — مقصورة عليك وحدك حتى مع «كل شيء».</b></div>
        </>)}
      </div>
    </div>
  </div>);
