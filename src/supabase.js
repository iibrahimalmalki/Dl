...
// رفع موثّق عبر دالة طرفية (Edge Function) تعمل بصلاحية الخدمة بعد التحقّق من هوية المستخدم.
// نتجاوز بهذا خللاً في تحقّق التخزين من توكن المستخدم كان يجعل الرفع يصل كزائر مجهول
...
export async function uploadAuthed(bucket,path,file,contentType="image/jpeg"){
  ...
  const res=await fetch(`${SUPA_URL}/functions/v1/field-evidence-upload`,{method:"POST",headers:{
